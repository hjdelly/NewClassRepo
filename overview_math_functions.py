...
print(abs(loss)) #abs = absolute value of loss-takes pure value and removes the negative sign <---20.25
print(math.floor(product_cost)) # returns 89 rounded down value-math floor returns an integer-you can call int as well <---89
print(math.ceil(product_cost))   # returns 90 upper rounded value-math ceiling-rounds up to next whole number <---90
print(abs(math.floor(loss)))  #math floor is called first then the absolute value <---21
print(round(product_cost)) #rounding to the nearest whole number 90-simply round to the nearest whole number <---90
